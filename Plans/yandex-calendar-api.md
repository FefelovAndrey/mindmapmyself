...
Данные передаются в формате **iCalendar** (`.ics`): компонент `VEVENT` — события (в UI Яндекса: вкладки **«Встреча»** и **«Задача»** в диалоге «Новое событие»), `VTODO` — элементы списка задач (`todos-*`).

> Для фичи Mind Map (BRD v1.1) нужен **VEVENT с типом «Задача»** в календаре событий (`events-*`), **не** VTODO.
...
### Событие с типом «Задача» (не встреча, не VTODO)

В веб-UI Яндекс Календаря диалог **«Новое событие»** имеет вкладки **«Встреча»** и **«Задача»**. Запись при этом попадает в календарь **событий** («Мои события» / `events-*`), а не в список задач `todos-*`.

Для Mind Map (см. `BRD-YandexCalendar-Feature1.md` v1.1) нужно создавать именно такой объект:

| Нужно | Не нужно |
|-------|----------|
| `VEVENT` в `events-<id>/` | `VTODO` в `todos-<id>/` |
| Тип UI: **Задача** | Тип UI: **Встреча** |

**Открытый вопрос (OQ-6 в BRD):** каким iCalendar/X-свойством CalDAV отличает вкладку «Задача» от «Встреча». Перед реализацией — экспериментальный `PUT` + проверка в UI.
